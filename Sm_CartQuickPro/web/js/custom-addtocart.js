	...
	'domReady!'
		...
		$.widget('cartQuickPro.ajaxCart',{
			...
			_initAjaxWishlist: function () {
				console.log("here1");
				$('.tocart').css('background', '#9FBB78!important');
				var _self = this , _options = _self.options;
						...
						if (_isValid){
							...
							if (_options.isProductView) {
								if (_form.has('input[type="file"]').length && _form.find('input[type="file"]').val() !== '') {
									...
									return  window.parent.location.href = window.parent.checkout.maximumCartUrl;
								}
								window.parent.ajaxCart._sendAjax(_oldAction, _params, true);
							}else{
								if (_form.has('input[type="file"]').length && _form.find('input[type="file"]').val() !== '') {
									_form.submit();
									 return setTimeout(function(){ _self._showLoader(); window.location.href = window.checkout.maximumCartUrl;}, 500);
								}
